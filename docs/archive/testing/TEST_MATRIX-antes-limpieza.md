...
Casos agregados durante el avance de Fase 4:

| ID | Módulo | Caso | Prioridad | Evidencia actual | Estado |
| --- | --- | --- | --- | --- | --- |
| ENV-012 | Integración | Conexión institucional secundaria deshabilitada por defecto | P0 | `InstitutionalDesignacionesClientTest` | CUBIERTA |
| INST-001 | Integración | Normalizar salida de `designaciones.f_asignaciones` en lectura `READ ONLY` | P0 | `InstitutionalDesignacionesClientTest` | CUBIERTA |
| INST-002 | Integración | Director solo consulta el programa de su carrera | P0 | `InstitutionalDesignacionesEndpointTest` | CUBIERTA |
| INST-003 | Integración | Vicerrectorado consulta `UATF` | P0 | `InstitutionalDesignacionesEndpointTest` | CUBIERTA |
| INST-004 | Integración | Confirmar filtrado real de gestión/periodo para `INF` | P0 | Salida institucional entregada | NEEDS_BUSINESS_CONFIRMATION |
| BACK-001 | Backend | Validaciones repetidas extraídas a Form Requests sin cambiar reglas | P0 | Suite de propuestas e importación | CUBIERTA |
| BACK-002 | Backend | Consultas de listado y edición separadas del controlador HTTP | P1 | `DesignacionesInterfazTest`, `PropuestaRosterTest` | CUBIERTA |
