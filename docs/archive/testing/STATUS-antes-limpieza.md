...
## Avance de Fase 4: adaptador de lectura institucional

Fecha de actualización: 2026-08-10

Se implementó el primer límite seguro para `designaciones.f_asignaciones`:

- `InstitutionalDesignacionesClient` usa una conexión PostgreSQL secundaria,
  parámetros enlazados y `SET TRANSACTION READ ONLY`.
- `InstitutionalDesignacionesController` expone
  `GET /institucional/designaciones` para usuarios autenticados autorizados.
- Los directores quedan restringidos al `sigla` de su carrera; Vicerrectorado
  puede consultar `UATF`.
- La conexión y la integración permanecen deshabilitadas por defecto; no se
  añadieron credenciales reales.
- La salida se normaliza sin convertirla todavía en docentes, materias, grupos
  ni horas, porque esas columnas no existen en el resultado observado.

| Comando | Resultado |
| --- | --- |
| `php artisan test tests/Unit/InstitutionalDesignacionesClientTest.php tests/Feature/InstitutionalDesignacionesEndpointTest.php --no-coverage` | OK; 5 pruebas, 18 aserciones |
| `composer test -- --env=testing` | OK; 83 pruebas, 464 aserciones |
| `vendor/bin/pint --test` | OK |
| `git diff --check` | OK |

La configuración institucional quedó centralizada en `config/institutional.php`;
`config/deployment.php` conserva únicamente opciones de despliegue y health.
Las validaciones repetidas de propuestas, asignaciones e importación viven en
Form Requests y respetan autorización antes de validar datos del recurso. Las
consultas de listado y edición se movieron a `PropuestaConsultaService`; el
controlador conserva rutas, respuestas y autorización HTTP.

Pendientes antes de cerrar la integración institucional: confirmar el filtrado
real de `INF/gestión/periodo`, estabilidad de `r_id`, estados y el origen de
los identificadores de docente/materia/grupo. No se inició aún la importación
ni la persistencia de datos institucionales.
