...
@section('title', 'Propuestas de Designación')

@section('content')
    <div class="max-w-6xl mx-auto space-y-6">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                <h1 class="text-xl font-bold text-gray-900">Propuestas de Designación</h1>
                <p class="text-sm text-gray-600 mt-1">Borradores y versiones enviados por su carrera.</p>
            </div>
        </div>

        @if($gestionActual)
            <section class="bg-white border border-gray-200 p-5 shadow-sm">
                <h2 class="text-sm font-bold text-gray-900">Nuevo borrador</h2>
                <form method="POST" action="{{ route('propuestas.crear') }}" class="mt-4 grid grid-cols-1 md:grid-cols-[1fr_180px_auto] gap-3 items-end">
                    @csrf
                    <div>
                        <label for="descripcion" class="block text-xs font-semibold text-gray-700 mb-1">Descripción</label>
                        <input id="descripcion" name="descripcion" value="{{ old('descripcion') }}" maxlength="255" class="w-full border border-gray-300 px-3 py-2 text-sm" placeholder="Propuesta de designación docente">
                    </div>
                    <div>
                    ...
                    <button type="submit" class="bg-[#00acac] text-white px-4 py-2 text-sm font-semibold hover:bg-[#008a8a]">Abrir borrador</button>
                </form>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-left text-xs uppercase text-gray-600">
                    <tr>
                        ...
                        <th class="px-4 py-3">Versiones</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($propuestas as $propuesta)
                        ...
                        <tr>
                            <td class="px-4 py-3 font-medium text-gray-900">{{ $propuesta->descripcion ?: 'Propuesta sin descripción' }}</td>
                            ...
                            <td class="px-4 py-3 text-right"><a class="text-[#007c7c] font-semibold hover:underline" href="{{ route('propuestas.editar', $propuesta) }}">Abrir</a></td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="px-4 py-8 text-center text-gray-500">Aún no existen propuestas para su carrera.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </section>
