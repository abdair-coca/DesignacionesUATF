...
export default function Copiar({ carreras, gestiones, periodos, docentes, filtros, filas: filasOrigen }) {
    ...
    const seleccionCompleta =
        filtros.gestion_origen_id && filtros.periodo_origen_id && filtros.gestion_destino_id && filtros.periodo_destino_id;

    const hayFilas = seleccionCompleta && filasOrigen !== null && data.filas.length > 0;
        ...
        <AppLayout title="Copiar designaciones de otra gestión">
            ...
            {!seleccionCompleta && (
                <p className="text-gray-500 text-sm">
                    Elegí gestión/periodo de origen y gestión/periodo de destino para ver qué se puede copiar.
                </p>
            )}
            ...
            {seleccionCompleta && filasOrigen !== null && filasOrigen.length > 0 && (
                <div className="space-y-6">
                    <div className="flex items-center gap-3">
                        ...
                        >
                            Copiar todas ({filasOrigen.length})
                        </button>
                        <span className="text-sm text-gray-400">o seleccioná individualmente abajo</span>
                    </div>
                    ...
                    {hayFilas && (
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <button
                                type="submit"
                                disabled={processing}
                                className="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-700 shadow-sm transition-colors hover:bg-gray-50 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50"
                            >
